...
void generate_key(char *receivedKey,char* outputKey)
{
    ...
    // char clientKey[] = "FHZmcJ9CyR8ucn3kEXP8jg==";
    char magicString[120]={0};
...
void sendHandShake(char requestKey[])
{
    ...
    strcat(printStuff,requestKey);

	generate_key(requestKey,returnKey);
    strcat(printStuff, returnKey);
...
int main()
{
    ...
    char clientKey[] = "RxmA/hPhedrK9hyJ3l4xhA==";
    sendHandShake(clientKey);
}